...
import { getUserMemoriesInfiniteQueryOptions } from "~/utils/server/memories";
    ...
    if (memories.length === 0) {
        return (
            <div className="max-w-4xl mx-auto space-y-6 pt-12">
                <div className="space-y-2">
                    <div className="flex items-center justify-center pt-12">
                        <p className="text-lg text-muted-foreground">No memories found for this user.</p>
                    </div>
                </div>
            </div>
        );
                ...
                {memories.map((memory, index) => (
                    <Link to="/memory-lanes/$id" params={{ id: memory.id }} key={memory.id}>
                        <Polaroid
                            ...
                            isSelected={false}
                            key={memory.id}
                            src={memory.memories?.[0]?.image || 'https://placehold.co/240x320'}
                            alt={memory.name}
                            caption={memory.name}
                            aspectRatio="square"

...
export const PublishedMemoryLanesFeed = memo<PublishedMemoryLanesFeedProps>(({
  ...
  const { data: memoryLanes, isLoading, error } = useQuery(
    getPublishedMemoryLanesQueryOptions(page, limit)
  );

  const memoryLanesWithMemories = useMemo(() => {
    return memoryLanes?.filter(lane => lane.memories && lane.memories.length > 0) || [];
  }, [memoryLanes]);
  ...
  if (error) {
    return (
      <div className={`text-center py-12 ${className}`}>
        <div className="text-destructive mb-2">Failed to load memory lanes</div>
  ...
  if (!memoryLanesWithMemories.length) {
    return (
      <div className={`text-center py-12 ${className}`}>
        <div className="text-muted-foreground mb-2">No published memory lanes found</div>
            ...
            <Card className="overflow-hidden transition-all duration-200 hover:shadow-lg hover:-translate-y-1">
              <CardHeader className="pb-3">
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  ...
                  <PublicationStatusBadge status={memoryLane.status} />
                </div>
              </CardHeader>
              <CardContent>
                {firstMemory && (
